...
function FindUser(props) {
  return (
      <div className="container-row border-bottom" style={{background:'#FFFFFF',width:'50%',marginTop:'5%', display:'flex', alignItems:'center'}}>
        <img src={require('../Icon/nophoto.png')} alt="Моя картинка" style={{width: '100px', height: '100px',caretColor:'transparent'}} />
        <div className='container' style={{marginLeft:'10px'}}>
          <h2 style={{textAlign:'left',fontFamily:"Livvic",fontSize:'20px',caretColor:'transparent'}}>{props.firstname},{props.lastname}</h2>
          <h3 style={{textAlign:'left',fontSize:'15px',color:'#8B8888',caretColor:'transparent'}}>ID: {props.userId}</h3>
          <p  style={{textAlign:'left',fontFamily:"Livvic",fontSize:'20px',caretColor:'transparent'}}>{props.about}</p>
        </div>
      </div>
  );
}
...
function FindUserList(props){
    const { volByName } = props;

    // Check if volByName is an array
    if (!Array.isArray(volByName)) {
...
export default function Volunteering() {
  ...
  function showByName()
  {
    ...
    const lastname = document.getElementById('search-input').value;
